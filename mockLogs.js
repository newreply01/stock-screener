...
async function mockLogs() {
    try {
        const now = new Date();
        const logs = [
            ['fetcher.js', 'SUCCESS', '盤後行情抓取完成 (等待下次 15:30 執行)'],
            ['news_fetcher.js', 'SUCCESS', '新聞更新完成 (等待下次整點執行)'],
            ['finmind_fetcher.js', 'SUCCESS', '基本面資料更新完成 (等待週六 04:00 執行)'],
            ['calc_health_scores.js', 'SUCCESS', '健診排行計算完成 (等待下次 16:00 執行)']
        ];

        for (const [script, status, message] of logs) {
            await pool.query(
                "INSERT INTO system_status (service_name, status, message, check_time) VALUES ($1, $2, $3, $4)",
                [script, status, message, now]
            );
