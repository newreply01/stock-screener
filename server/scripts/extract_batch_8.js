...
async function extractSingle(symbol) {
    ...
    try {
        console.log(`Extracting ${symbol} (Batch 8)...`);
...
async function run() {
    ...
    const symbols = allSymbols.slice(318, 418);
    const batchResults = [];
    try {
        for (const s of symbols) {
            batchResults.push(await extractSingle(s));
        }
        fs.writeFileSync(__dirname + '/batch_context_8.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 8 context (${symbols.length} symbols) saved`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
