...
async function extractSingle(symbol) {
    ...
    try {
        console.log(`Extracting ${symbol} (with 7-factor)...`);
...
async function run() {
    ...
    try {
        ...
        fs.writeFileSync(__dirname + '/batch_context_6.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 6 context (${symbols.length} symbols) saved`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
