...
async function extractSingle(symbol) {
    ...
    try {
        console.log(`Extracting ${symbol} (Batch 10)...`);
...
async function run() {
    ...
    // Indices 518 to 1018 (500 symbols)
    const symbols = allSymbols.slice(518, 1018);
    const batchResults = [];
    try {
        for (const s of symbols) {
            batchResults.push(await extractSingle(s));
        }
        fs.writeFileSync(__dirname + '/batch_context_10.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 10 context (${symbols.length} symbols) saved to batch_context_10.json`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
