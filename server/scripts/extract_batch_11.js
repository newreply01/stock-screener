...
async function extractSingle(symbol) {
    ...
    try {
        console.log(`Extracting ${symbol} (Batch 11)...`);
...
async function run() {
    ...
    // Next batch: 1018 to 1518 (500 symbols)
    const symbols = allSymbols.slice(1018, 1518);
    const batchResults = [];
    try {
        for (const s of symbols) {
            batchResults.push(await extractSingle(s));
        }
        fs.writeFileSync(__dirname + '/batch_context_11.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 11 context (${symbols.length} symbols) saved to batch_context_11.json`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
