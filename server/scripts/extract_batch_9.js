...
async function extractSingle(symbol) {
    ...
    try {
        console.log(`Extracting ${symbol} (Batch 9)...`);
        ...
        // Updated to include Bollinger Bands for %b calculation
...
async function run() {
    ...
    // Next batch: 418 to 518 (100 symbols)
    const symbols = allSymbols.slice(418, 518);
    const batchResults = [];
    try {
        for (const s of symbols) {
            batchResults.push(await extractSingle(s));
        }
        fs.writeFileSync(__dirname + '/batch_context_9.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 9 context (${symbols.length} symbols) saved to batch_context_9.json`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
