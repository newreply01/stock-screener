...
async function extractSingle(symbol) {
    ...
    try {
        console.log(`Extracting ${symbol} (Batch 12)...`);
...
async function run() {
    ...
    // Batch 12: 1518 to 2018 (500 symbols)
    const symbols = allSymbols.slice(1518, 2018);
    const batchResults = [];
    try {
        for (const s of symbols) {
            batchResults.push(await extractSingle(s));
        }
        fs.writeFileSync(__dirname + '/batch_context_12.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 12 context (${symbols.length} symbols) saved to batch_context_12.json`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
