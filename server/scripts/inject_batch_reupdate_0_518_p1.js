...
async function injectBatchPart(partNum, startIndex, endIndex) {
    console.log(`🚀 Starting injection for Re-update Part ${partNum} (Indices ${startIndex}-${endIndex})...`);
    ...
    console.log(`📊 Batch data length calculated: ${batchData.length}`);
    ...
    for (let i = 0; i < batchData.length; i++) {
        ...
        if (!item || !item.symbol) {
            console.log(`⚠️ Item at ${i} is invalid`);
            continue;
        }
        const symbol = item.symbol;
        console.log(`Working on symbol: ${symbol}`);
        
        process.stdout.write(`[⏳] RU-P${partNum} ${symbol} (${i + 1}/${batchData.length})... `);
        ...
        try {
            ...
            // This will overwrite existing records due to upsert logic in ai_service.js
    ...
    console.log(`✅ Loop finished normally.`);
    console.log(`\n✨ Re-update Part ${partNum} Complete: ${successCount} Success, ${failCount} Fail`);
    await pool.end();
    console.log(`🛢️ Pool closed.`);
}

// Part 1: 0 to 100
injectBatchPart(1, 0, 100).then(() => process.exit(0));
