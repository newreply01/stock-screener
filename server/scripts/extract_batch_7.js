...
async function run() {
    ...
    const symbols = allSymbols.slice(218, 318);
    const batchResults = [];
    try {
        for (const s of symbols) {
            batchResults.push(await extractSingle(s));
        }
        fs.writeFileSync(__dirname + '/batch_context_7.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 7 context (${symbols.length} symbols) saved`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
