...
async function run() {
    const symbols = ['3481', '2409', '2337', '2488', '2344'];
    const batchResults = [];
    try {
        ...
        fs.writeFileSync(__dirname + '/batch_context_1.json', JSON.stringify(batchResults, null, 2));
        console.log('✅ Batch context saved to batch_context_1.json');
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
