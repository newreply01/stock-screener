...
async function extractSingle(symbol) {
    ...
    try {
        console.log(`Extracting ${symbol} (Batch 13 - Final)...`);
...
async function run() {
    ...
    // Final batch: 2018 to end (2506)
    const symbols = allSymbols.slice(2018, 2506);
    const batchResults = [];
    try {
        for (const s of symbols) {
            batchResults.push(await extractSingle(s));
        }
        fs.writeFileSync(__dirname + '/batch_context_13.json', JSON.stringify(batchResults, null, 2));
        console.log(`✅ Batch 13 context (${symbols.length} symbols) saved to batch_context_13.json`);
    } catch (e) {
        console.error(e);
    } finally {
        await pool.end();
