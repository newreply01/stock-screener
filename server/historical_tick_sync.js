    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 429 || res.status === 422) {
                console.warn(`[FinMind] Rate limited/Error (${res.status}) on Token #${currentTokenIndex + 1}`);
                if (rotateToken(`HTTP ${res.status}`)) return fetchFinMind(dataset, data_id, start_date);
                console.warn(`Waiting 60s...`);
                await sleep(60000);
        ...
        try {
            ...
            for (const row of batch) {
                ...
                await client.query(`
                    ...
                    ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 50, 50, '[]'::jsonb)
...
// --- Exported batch function for scheduler use ---
async function syncHistoricalMinuteBatch(startDate, limit = 100) {
    console.log(`[Historical Sync] Batch run: start=${startDate}, limit=${limit}`);
    ...
    let symbols = res.rows.map(r => r.symbol);
    if (symbols.length === 0) {
        const fallback = await query(`SELECT symbol FROM stocks WHERE symbol ~ '^[0-9]{4}$' ORDER BY symbol ASC LIMIT $1`, [limit]);
        symbols = fallback.rows.map(r => r.symbol);
    }

    console.log(`[Historical Sync] Processing ${symbols.length} symbols from ${startDate}...`);
    for (const sym of symbols) {
        const data = await fetchFinMind('TaiwanStockPriceMinute', sym, startDate);
        if (data && data.length > 0) {
            const dbClient = await pool.connect();
            try {
                await bulkUpsertRealtimeTicks(dbClient, data);
            } finally {
                dbClient.release();
            }
        }
    ...
    console.log(`[Historical Sync] Batch complete.`);
}

// --- CLI mode: run directly ---
async function syncHistoricalMinute() {
    if (process.env.ENABLE_CRAWLER !== 'true') {
        console.log('Crawler is disabled. Set ENABLE_CRAWLER=true to enable.');
        process.exit(0);
    }

    const args = process.argv.slice(2);
    const startDate = args.find(a => a.startsWith('--start='))?.split('=')[1] || '2025-01-01';
    const limit = parseInt(args.find(a => a.startsWith('--limit='))?.split('=')[1] || '20');

    try {
        await syncHistoricalMinuteBatch(startDate, limit);
    } catch (e) {
        console.error('Fatal Sync Error:', e);
    } finally {
        console.log('[Historical Sync] Batch finished.');
        process.exit(0);
    }
}
...
module.exports = { syncHistoricalMinuteBatch };
